...
class ObservationManagerDistillation(ObservationManager):
        ...
        for group_name, group_cfg in group_cfg_items:
            ...
            for term_name, term_cfg in group_cfg_items:
                ...
                if term_name == "scan_dot":
                    scan_dots = True
                    print("I have scan_dots")
                if term_name == "depth_image":
                    is_depth = True
                    print("I have depth_image")
              ...
              #  print(f"term cfg {term_cfg}")
                ...
                # helper variable 
                any_vision_input = scan_dots or is_depth

                if (scan_dots or is_depth):
                    if obs_term_id == 0 and group_name == "policy":
                        # allow scan_dot or depth_image in the first slot
                        valid_names = ["scan_dot", "depth_image"]
                        if term_name not in valid_names:
                            raise TypeError(f"Expected scan_dot or depth_image at index 0, got {term_name}")
                        
                        # update config dimension tracker
                    if term_name == "depth_image":
                        # You might want to store this in a new cfg variable like n_depth
                        print(f"Depth Image Detected with dims: {obs_dims[1:]}")
               ...
               # print(f" n_scan before rewrite{self._env.cfg.n_scan}")
                ...
                if not scan_dots and self._env.cfg.n_scan != 0:
                    raise ValueError(f"An observation with term name 'scan_dot' was not found, yet the env cfg param 'n_scan' is not 0, it is {self._env.cfg.n_scan}")

                if any_vision_input:
                    if obs_term_id == 0 and group_name == "policy": 
                        valid_vision_terms = ["scan_dot", "depth_image"]
                        if term_name not in valid_vision_terms:
                        ...
                        if term_name == "scan_dot":
                            ...
                            print(f"Verified depth image: {obs_dims[1:]}")
                elif obs_term_id == (1 - (not any_vision_input)) and group_name == "policy":
                    if term_name != "base_lin_vel":
                        raise TypeError(f"I have the wrong obs term in the {obs_term_id} obs group slot. I have {term_name} where it should be 'base_lin_vel'")
                    if self._env.cfg.n_priv != obs_dims[1]:
                        raise ValueError(f"n_priv in the {obs_term_id} obs group has changed to something not equal to env.cfg set val. Check obs group order")
                ...
                elif obs_term_id == (2 - (not any_vision_input)) and group_name == "policy":
                    if term_name != "priv_latent_gains_stiffness":
                        raise TypeError(f"I have the wrong obs term in the {obs_term_id} obs group slot. I have {term_name} where it should be 'priv_latent_gains_stiffness'")
                    if self._env.cfg.n_priv_latent_gains_stiffness != obs_dims[1]:
                        raise ValueError(f"n_priv_latent_gains_stiffness in the {obs_term_id} obs group has changed to something not equal to env.cfg set val. Check obs group order")
                    self._env.cfg.n_priv_latent_gains_stiffness = obs_dims[1]
                    print(f"n_priv_latent_gains_stiffness {obs_dims[1]}")

                elif obs_term_id == (3 - (not any_vision_input)) and group_name == "policy":
                    if term_name != "priv_latent_gains_damping":
                        raise TypeError(f"I have the wrong obs term in the {obs_term_id} obs group slot. I have {term_name} where it should be 'priv_latent_gains_damping'")
                    if self._env.cfg.n_priv_latent_gains_damping != obs_dims[1]:
                        raise ValueError(f"n_priv_latent_gains_damping in the {obs_term_id} obs group has changed to something not equal to env.cfg set val. Check obs group order")
                    self._env.cfg.n_priv_latent_gains_damping = obs_dims[1]
                    print(f"n_priv_latent_gains_damping {obs_dims[1]}")

                
                elif obs_term_id == (4 - (not any_vision_input)) and group_name == "policy":
                    if term_name != "priv_latent_mass":
                        raise TypeError(f"I have the wrong obs term in the {obs_term_id} obs group slot. I have {term_name} where it should be 'priv_latent_mass'")
                    if self._env.cfg.n_priv_latent_mass != obs_dims[1]:
                        raise ValueError(f"n_priv_latent_mass in the {obs_term_id} obs group has changed to something not equal to env.cfg set val. Check obs group order")
                    self._env.cfg.n_priv_latent_mass = obs_dims[1]
                    print(f"n_priv_latent_mass {obs_dims[1]}")

                elif obs_term_id == (5 - (not any_vision_input)) and group_name == "policy":
                    if term_name != "priv_latent_com":
                        raise TypeError(f"I have the wrong obs term in the {obs_term_id} obs group slot. I have {term_name} where it should be 'priv_latent_com'")
                    if self._env.cfg.n_priv_latent_com != obs_dims[1]:
                        raise ValueError(f"n_priv_latent_com in the {obs_term_id} obs group has changed to something not equal to env.cfg set val. Check obs group order")
                    self._env.cfg.n_priv_latent_com = obs_dims[1]
                    print(f"n_priv_latent_com {obs_dims[1]}")

                
                elif obs_term_id == (6 - (not any_vision_input)) and group_name == "policy":
                    if term_name != "priv_latent_friction":
                        raise TypeError(f"I have the wrong obs term in the {obs_term_id} obs group slot. I have {term_name} where it should be 'priv_latent_friction'")
                    if self._env.cfg.n_priv_latent_friction != obs_dims[1]:
                        raise ValueError(f"n_priv_latent_friction in the {obs_term_id} obs group has changed to something not equal to env.cfg set val. Check obs group order")
                    self._env.cfg.n_priv_latent_friction = obs_dims[1]
                    print(f"n_priv_latent_friction {obs_dims[1]}")

                elif obs_term_id == (7 - (not any_vision_input)) and group_name == "policy":
                    if term_name != "joint_pos_rel":
                        raise TypeError(f"I have the wrong obs term in the {obs_term_id} obs group slot. I have {term_name} where it should be 'joint_pos_rel'")
                    # proprioception vector has pos and vel so divide by 2, also account for history length
                    if self._env.cfg.n_proprio/2 != obs_dims[1]/self._env.cfg.history_len:
                        raise ValueError(f"n_proprio in the {obs_term_id} obs group has changed to something not equal to env.cfg set val. Check obs group order")
                    #self._env.cfg.n_priv_latent = obs_dims[1]
                    print(f"joint_pos_rel {obs_dims[1]}")

                elif obs_term_id == (8 - (not any_vision_input)) and group_name == "policy":
                    if term_name != "joint_vel_rel":
                        raise TypeError(f"I have the wrong obs term in the {obs_term_id} obs group slot. I have {term_name} where it should be 'joint_vel_rel'")
                    if self._env.cfg.n_proprio/2 != obs_dims[1]/self._env.cfg.history_len:
                        raise ValueError(f"n_proprio in the {obs_term_id} obs group has changed to something not equal to env.cfg set val. Check obs group order")
                    #self._env.cfg.n_priv_latent = obs_dims[1]
