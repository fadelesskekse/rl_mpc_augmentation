...
from ..marker.visualization_marker import (
...
class BlindMPCAction(ActionTerm):
    ...
    def __init__(self, cfg: mpc_actions_cfg.BlindMPCActionCfg, env: ManagerBasedEnv):
        ...
        foot_idx = [i for i in range(len(body_names)) if body_names[i] in ["left_ankle_roll_link", "right_ankle_roll_link"]]
    ...
    def action_dim(self) -> int:
        ...
        - swing foot height (R^1) - NOT USED
        - swing trajectory control points (R^1) - NOT USED
        """
        return 7 #9
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        #only clamp the lin acc terms. The range I specify for dt mpc is a + - value from nominal, 
        self._processed_actions[:] = torch.clamp(self._raw_actions, self._action_lb, self._action_ub)
        ...
        sampling_time = self.cfg.nominal_mpc_dt + self._processed_actions[:, sampling_time_idx]
    ...
    def apply_actions(self):
        # obtain state
        self._get_state() #Gets robot states from robot api.
        self.mpc_controller.update_state(self.state) #sends the current states to the mpc controller 
