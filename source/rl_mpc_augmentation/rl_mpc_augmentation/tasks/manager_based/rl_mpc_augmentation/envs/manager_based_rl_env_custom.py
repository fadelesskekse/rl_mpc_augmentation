...
from .manager_based_env_custom import ManagerBasedEnvCustom
...
class ManagerBasedRLEnv(ManagerBasedEnvCustom, gym.Env):
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        """
        # process actions
        #print(f"actions before aciton_manager process in manager_rl {action.to(self.device)}")
        ...
        for _ in range(self.cfg.decimation):
            ...
            #print(f"actions after manager_rl_apply: {self.action_manager.action}")
    ...
    def _reset_idx(self, env_ids: Sequence[int]):
        ...
        #print(f"obs_info_dict: {info}")
        # -- action manager
        info = self.action_manager.reset(env_ids)
        self.extras["log"].update(info)
        #print(f"action_info_dict: {info}")
        # -- rewards manager
        info = self.reward_manager.reset(env_ids)
        self.extras["log"].update(info)
        #print(f"reward_info_dict: {info}")
        # -- curriculum manager
        info = self.curriculum_manager.reset(env_ids)
        self.extras["log"].update(info)
        #print(f"curr_info_dict: {info}")
        # -- command manager
        info = self.command_manager.reset(env_ids)
        self.extras["log"].update(info)
        #print(f"command_info_dict: {info}")
        # -- event manager
        info = self.event_manager.reset(env_ids)
        self.extras["log"].update(info)
       # print(f"event_info_dict: {info}")
        # -- termination manager
        info = self.termination_manager.reset(env_ids)
        self.extras["log"].update(info)
       # print(f"term_info_dict: {info}")
        # -- recorder manager
        info = self.recorder_manager.reset(env_ids)
        self.extras["log"].update(info)
        #print(f"recorder_info_dict: {info}")
